// "Bobby AI" chat endpoint (Next.js route handler; ports src/pages/api/chat.ts).
//
// Flow per request:
//   1. validate + rate-limit (DB-backed, per session)
...
//   5. log the turn to Supabase `conversations` (via waitUntil, so it survives after the
//      response closes on Vercel serverless)
//
// Secrets are read server-side only (process.env). GEMINI_API_KEY is always required
// (embeddings); ANTHROPIC_API_KEY only when CHAT_PROVIDER=anthropic.
...
// node:crypto pins this to the Node runtime; streaming + logging must not be cached.
...
const DEVICE_ID_SALT = process.env.DEVICE_ID_SALT;
...
export async function POST(request: Request) {
  ...
  // deployed function, null under local dev); `device_hash` is a one-way hash of the
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (!answer) {
          controller.enqueue(
            encoder.encode('Sorry, something went wrong on my end.'),
          );
        }
      } finally {
        controller.close();
        // Log the turn without blocking the response. waitUntil keeps the Vercel
        // function alive until the insert resolves (locally the promise just runs).
        const logTurn = (async () => {
        ...
        try {
          ...
          // Not running on Vercel (e.g. local dev) — the promise still runs to completion.
        }
      }
    },
  });
