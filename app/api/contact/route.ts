// Contact-form endpoint (Next.js route handler; ports src/pages/api/contact.ts).
// Delivers the "Shoot me an email" dialog to Bobby's inbox via Resend.
//
// Flow per request:
...
// must be the Gmail the Resend account was created with.
...
const CONTACT_TO = process.env.CONTACT_TO || 'bobbymul93@gmail.com';
const CONTACT_FROM =
...
export async function POST(request: Request) {
  ...
  if (!RESEND_API_KEY) {
    ...
    return json(500, {
      error:
        'The contact form is not configured yet. Please reach out via GitHub or LinkedIn.',
    });
  }
